...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.find_by_sql("Select * from movies WHERE release_date > 2002 order by release_date DESC")
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick"
  Movie.create(title: "Awesome Flick")
  changing = Movie.find_by(:title => "Awesome Flick")
  changing.update(:title => "Even Awesomer Flick")
  changing.save
end
...
def can_update_using_update_method
  ...
  again = Movie.find_by(:title => "Wat?")
  again.update(:title => "Wat, huh?")
end
